...
const calculateMCFaddenIncMean = (inclinations: Array<number>) => {
  /*
  ...
  */
  ...
  const absInclinations = inclinations.map((inc) => Math.abs(inc));
  ...
  if (gaussparsRes.MI < 30) {
    // mean inc < 30, returning gaussian mean'
    fpars = {
