...
const dataToStereoDIR = (
  ...
  if (statistics) {
    ...
    const [declination, inclination] = centeredByMean
      ? centeredDirection.toArray()
      : direction.toArray();
    const meanXYData = dirToCartesian2D(declination - 90, inclination, graphSize);
    const confidenceCircle = createStereoPlaneData(
      centeredByMean ? centeredDirection : direction,
      graphSize,
      MAD,
    );
    const confidenceCircle45 = createStereoPlaneData(
      centeredByMean ? centeredDirection : direction,
      graphSize,
      45,
    );
    const greatCircle = createStereoPlaneData(
      centeredByMean ? centeredDirection : direction,
      graphSize,
    );
  ...
  const dotsData: DotsData = directionalData.map((di, index) => {
    ...
    let a95 = 0;
    if (directions[index]) {
      a95 = reference === 'stratigraphic' ? directions[index].MADstrat : directions[index].MADgeo;
    }
    const confidenceCircle = createStereoPlaneData(direction, graphSize, a95);
    const greatCircle =
      codes[index] === 'gc' || codes[index] === 'gcn'
        ? createStereoPlaneData(direction, graphSize)
        : undefined;
